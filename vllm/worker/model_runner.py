...
class ModelRunner:
    ...
    def _nobe_prepare_model_input(
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            for seq_id in seq_ids:
                ...
                # NOTE(bong-furisa)
                # Commented out for No Batch Expansion SPS.
                # seq_len = min(
                #     seq_data.get_len(),
                #     context_len + seq_group_metadata.token_chunk_size)
                ...
                if (self.sliding_window is not None and not is_prompt):
                    raise AssertionError(
                        "(bong-furiosa):"
                        "Currently, sliding window is not supported.")
                    curr_sliding_window_blocks = sliding_window_blocks
                    if self.scheduler_config.use_v2_block_manager:
                        # number of elements in last block
                        suff_len = seq_len % self.block_size
                ...
                if is_prompt:
                    ...
                    # NOTE(bong-furiosa)
                    # Commented out for No Batch Expansion SPS, where
                    # query_len can exceed 1.
                    # assert query_len == 1, (
                    #     "seq_len: {}, context_len: {}, query_len: {}".format(
                    #         seq_len, context_len, query_len))
                    num_decode_tokens += query_len
                ...
                if mm_data is not None:
                    raise AssertionError(
                        "(bong-furiosa):"
                        "Currently, multi modal is not supported.")
                    # Process multi-modal data
                    if self.multi_modal_input_processor is None:
                        raise ValueError(
                            "Multi-modal inputs are only supported by "
        ...
        # Modified for No Batch Expansion SPS.
        # batch_size = len(input_tokens)
        batch_size = len(input_tokens) if num_speculative_tokens == 0 else len(
            input_tokens) // (num_speculative_tokens + 1)
        max_query_len = max(query_lens)
        max_prefill_seq_len = max(prefill_seq_lens, default=0)
        max_decode_seq_len = max(decode_seq_lens, default=0)
        ...
        if self.attn_backend.get_name() == "flashinfer":
            ...
            qo_indptr = [
                batch_idx * (num_speculative_tokens + 1)
                for batch_idx in range(batch_size + 1)
            ]
            qo_indptr_tensor = torch.tensor(qo_indptr,
                                            dtype=torch.int,
                                            device=self.device)
            paged_kv_indptr_tensor = torch.tensor(paged_kv_indptr,
                                                  dtype=torch.int,
        ...
        elif self.attn_backend.get_name() == "flash-attn":
            context_lens_tensor = torch.tensor(context_lens,
                                               dtype=torch.int,
                                               device=self.device)
            query_lens_tensor = torch.tensor(query_lens,
    ...
    def nobe_prepare_input_tensors(
        self,
        seq_group_metadata_list: Optional[List[SequenceGroupMetadata]],
        num_speculative_tokens: int,
    ) -> Tuple[torch.Tensor, torch.Tensor, AttentionMetadata, SamplingMetadata,
        ...
        if self.is_driver_worker:
            ...
            (
                ...
                num_decode_tokens,
                num_prefills,
            ) = self._nobe_prepare_model_input(seq_group_metadata_list,
    ...
    def execute_model(
        ...
        # NOTE(bong-furiosa)
        # Generate input tensors for prefill or draft model decoding.
        if num_speculative_tokens == 0:
            (input_tokens, input_positions, attn_metadata, sampling_metadata,
             lora_requests, lora_mapping, multi_modal_kwargs
             ) = self.prepare_input_tensors(seq_group_metadata_list)
        # NOTE(bong-furiosa)
        # Generate input tensors for target model decoding.
        else:
            (input_tokens, input_positions, attn_metadata, sampling_metadata,
             lora_requests, lora_mapping,
             multi_modal_kwargs) = self.nobe_prepare_input_tensors(
                 seq_group_metadata_list, num_speculative_tokens)
